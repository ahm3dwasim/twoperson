...
def _dispatch(args) -> int:
    ...
    if args.cmd in ("verify", "publish"):
        ...
        # `inbox.prepare_packet` is the ONE pipeline both `verify` and `publish` run: it validates,
        # derives+stamps diff evidence, and applies every ship-gate check — the same checks a caller
        # of the library directly (bypassing this CLI entirely) gets from `inbox.publish` itself.
        # This command prints its notes and, for `verify`, stops here without writing anything.
        try:
            validated, notes = inbox.prepare_packet(packet, derive=args.derive)
        except PacketError as exc:
            return _fail(f"packet rejected — {exc}")
        for note in notes:
            print(note, file=sys.stderr)
        if args.cmd == "verify":
            print(f"ok: packet {validated['packet_id']} is valid (nothing was written)")
            return EXIT_OK
        ...
        if args.derive and gitfacts.concrete(validated["git"]["head_sha"]):
            cites = check_citations(Path.cwd(), validated)
        ...
        try:
            path = inbox.publish(packet, derive=args.derive)
        # Named, not `OSError` — see `_emit_signal`: the refusal is a `PacketError`, and the only
        # `OSError` the primitive still lets past is the documented lost race.
        except (PacketError, FileNotFoundError) as exc:
